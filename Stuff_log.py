...
def login():
    if var_id.get() == "" or var_password.get() == "":
        ...
        try:
            conn = mysql.connector.connect(host="localhost", username="root", password="1601", database="hotel")
            my_cursor = conn.cursor()
            query = "select * from st_log where ID=%s and Password=%s"
            value = (var_id.get(), var_password.get(),)
            my_cursor.execute(query, value)

            rows = my_cursor.fetchone()
            if len(rows) != 0:
                stuff = messagebox.askyesno("YesNo", "Are you sure to login?")
                if stuff > 0:
                    root.destroy()
...
main_frame.place(x=0, y=140, width=500, height=300)

canv = Canvas(main_frame, width=497, height=300, bg="white")
canv.place(x=0, y=0)

lebelframeleft = LabelFrame(main_frame, text="Stuff LogIn", bd=2, relief=RIDGE, font=("times new roman", 15, "bold", "italic"), padx=2)
lebelframeleft.place(x=120, y=30, width=260, height=200)
